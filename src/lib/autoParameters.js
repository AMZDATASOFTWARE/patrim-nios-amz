...
export async function getParameterSuggestion(payload) {
  try {
    ...
    return {
      ...
      found: true,
      value: data.value,
      suggested_value: data.suggested_value ?? data.value,
      unit: data.unit || '',
      label: data.label || '',
      field: data.field || payload?.field_name || '',
      basis: data.basis || payload?.domain || '',
      identified_classification: data.identified_classification || '',
      explanation: data.explanation || data.rationale || '',
      rationale: data.rationale || data.explanation || '',
